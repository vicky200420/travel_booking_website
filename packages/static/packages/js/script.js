...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (resultsContainer && resultsContainer.children.length === 0) {
    ...
    setTimeout(function () {
      var container = document.getElementById('packageResults');
      if (!container) return;
      var stillLoading = container.querySelectorAll('.skeleton-card').length > 0;
      var hasContent = container.querySelector('.package-card, .empty-state');
      if (stillLoading && !hasContent) {
        ...
        desc.textContent = 'We could not load holiday packages at the moment. Please try again.';
